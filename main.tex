...
\eq{\label{Lambdasharptilde-2}g\br{\fr{(\L^{t})^{\~\sharp}V}{f},Y}&=\fr{h}{f}\br{(\L^{t})^{\~\sharp}V,\cW^{-1}(Y)}\\
...
=&(\log\p)'' \dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	\hp{=}&+\fr 2f\tr_{d_{h}f}(\tr_{h}(A\ast A))+\fr 2f \tr(d_{\cW}f\circ (A\circ\L^{\sharp}-\L^{\sharp}\circ A))\\
	\hp{=}&+\s\br{1-\fr{\tr_{d_{h}f}h(\cdot,\cdot)}{f}}\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})+\fr 2f \tr_{d_{h}f}\overline{\mrm{Rm}}(\cdot,\dot{x},\dot{x},\cW(\cdot))\\
...
\eq{\label{Ev-u-5-new}\dot{u}&-\tr_{d_{h}f}\hat{\n}^{2}_{\~g}u-\tr_{d_{h}f}C(\cdot,\grad_{\~g}u,\cdot)\\
		&+\fr 12 \tr_{d_hf} \left[\overline{\mrm{{Rm}}}(\cdot, \grad_{\~g}u)(\cdot)+ \overline{\mrm{Rm}}(\cdot, \cdot)\grad_{\~g}u\right]^{\dot{x}}\\
	=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\br{\overline{\mrm{Rm}}(\cdot,\dot{x})\dot{x}}^{\top}}+\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})\\
	&-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}.}		
...
\eq{\label{Ev-u-4-new}\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})=&\fr 2f\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr 2f\tr (d_Wf\circ\L^{\sharp}\circ A)\\						&+\fr 2f\tr_{d_{h}f}\br{\tr_{h} (A\ast A)},}
where we used
\begin{align*}
\tr_{d_{h}f}\L(A(\cdot),\cdot)=\tr_{d_hf}g(\L^{\sharp}\circ A(\cdot),\cdot))=\tr (d_Wf\circ\L^{\sharp}\circ A).
\end{align*}
  \item \cref{Lambdasharptilde} implies that
\eq{\label{Ev-u-2-new}&-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	=~&\fr{1}{f}\tr_{d_{h}f}\L(A(\cdot),\cdot)-\fr{2}{f}\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr{\s}{f}\tr_{d_{h}f}h(\cdot,\cdot)\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})\\
...
\eq{\label{Ev-u-1-new}&(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\ =&\fr{2}{f}\tr (d_{\cW}f\circ A\circ{\L^{\sharp}})-\fr{1}{f}\tr(d_{\cW}f\circ\L^{\sharp}\circ A)-\fr 1f\tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
...
\eq{\fr{1}{f^{2}}\tr_{d_{h}f}\overline{\mrm{Rm}}((\L^{t})^{\~\sharp}V,\cdot,\nu,\cdot)&=-\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}\br{\nu,\cdot,\cdot,\fr{(\L^{t})^{\~\sharp}V}{f}}\\
        ...
        &=-\fr 1f \tr_{d_{h}f}g\br{\mc{W}^{-1}\circ(\L^{t})^{\sharp}(V),\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}}\\
		&=-\fr 1f \tr_{d_{h}f}\L(\cW^{-1}(\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}),V).}
\end{itemize}
Putting these last three items all together gives
\eq{\label{Ev-u-4-new}&\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	&+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\
	=~& \fr 2f \tr(d_{\cW}f\circ (A\circ \L^{\sharp}-\L^{\sharp}\circ A))-\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
	\hp{=}~&+\fr 1f\tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\cdot,\dot{x})+\s\br{1-\fr{\tr_{d_{h}f}h(\cdot,\cdot)}{f}}\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})\\
	\hp{=}~&-\fr{1}{f}\tr_{d_{h}f}\overline{\mrm{Rm}}(\dot{x},\cdot,\dot{x},\cW(\cdot))+\fr 1f \tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\dot{x},\cdot)+\fr 2f\tr_{d_{h}f}\br{\tr_{h} (A\ast A)}.}
