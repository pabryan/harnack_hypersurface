...
\Theo{rem}{}{Fix a subgroup $G$ of linear isometries that $\mathbb{H}^n/G$ is compact (e.q., $G$ acting freely co-compactly on $\mathbb{H}^n$). Let $\mathcal{F}$ denote the interior of $\{\langle x,x\rangle\leq 0, x_0\geq 0\}$. A convex closed proper subset of $\mathcal{F}$ that is $G$-invariant is called $G$-Fuchsian convex body. A $G$-Fuchsian convex body has a positive support function. A Fuchsian hypersurface is the boundary of a Fuchsian convex body. Suppose now $M$ is a standard (under the group $G$), spacelike, strictly convex hypersurface that is contained in $\mathcal{F}$. Since $\nu:M\to \mathbb{H}^n$ is a diffeomorphism, $\tilde{\nu}:M/G\to \mathbb{H}^n/G,~ [x]\mapsto [I_{\ast}\nu(x)]$ is a diffeomorphism. Therefore $\mathbb{H}^n/G$ is a compact manifold. Hence $M$ is a $G$-Fuchsian hypersurface with positive support function.}
...
\eq{\label{Speed-1}\fr{\del f}{\del g_{ij}}X_{ij}=-f^{mi}h^{j}_{m}X_{ij},\quad \fr{\del^{2}f}{\del h_{ij}\del g_{kl}}=-(g^{kj}f^{il}+f^{ij,km}h^{l}_{m})X_{ij}Y_{kl},}
...
\eq{\label{Speed}f\cn \R_{+}\x UN \x \Sigma^{0,2}_{+}(M)\x\Sigma^{0,2}_{+}(M)&\ra \R\\
...
\eq{\label{Ev-Tangent-1}\del_{t}\br{b^{ki}f_{;i}}=fb^{ki}u_{;i}+ub^{ki}f_{;i}+A^{k}_{j}b^{jm}f_{;m}-\L_{ij}b^{ik}b^{jm}f_{;m}.}
}
\pf{
We calculate
\eq{u_{;i}&=\del_{t}\br{\fr{h_{ij}}{f}b^{jm} f_{;m}}\\
        &=\fr{h_{ij}}{f}\del_{t}\br{b^{jm}f_{;m}}-\fr{u}{f}f_{;i}-\fr{1}{f}h_{ir}A^{r}_{j}b^{jm}f_{;m}+\fr{1}{f}\L_{ij}b^{jm}f_{;m}.}
Multiplying both sides by $fb^{ki}$ and rearranging the terms yields the result.
}
\Theo{lemma}{Ev-speed}{
Along the flow \eqref{Flow} there holds
\eq{\label{Ev-speed-1}\ddot{x}=u\dot{x}-fb^{ik}u_{;i}x_{;k}+\L_{ij}b^{jm}b^{ik}f_{;m}x_{;k}.}
}
...
\eq{\ddot{x}&=-\s \dot{f}\nu-\dot{x}_{;k}b^{ki}f_{;i}-x_{;k}\del_{t}\br{b^{ki}f_{;i}}\\
        ...
        &\hp{=}-A^{k}_{j}b^{jm}f_{;m}x_{;k}+\L_{ij}b^{jm}b^{ik}f_{;m}x_{;k}\\
        &=u\dot{x}-fb^{ki}u_{;i}x_{;k}+\L_{ij}b^{jm}b^{ik}f_{;m}x_{;k}.}
}
\Theo{lemma}{Ev-A}{
Under the flow \eqref{Flow} the tensor $(A^{i}_{j})$ satisfies
\eq{\dot{A}^{i}_{j}&=A^{k}_{j}A^{i}_{k}+b^{ki}f_{;k}u_{;j}+uA^{i}_{j}+\br{fb^{il}u_{;l}}_{;j}\\
                &\hp{=}-\br{\L_{kl}b^{lm}b^{ki}f_{;m}}_{;j}+\-R_{\a\b\g\d}x^{\a}_{;r}\dot{x}^{\b}x^{\g}_{;j}\dot{x}^{\d}g^{ri}.}
}
\pf{
...
\eq{\dot{A}^{k}_{i}x^{\a}_{;k}&=A^{k}_{i}A^{l}_{k}x^{\a}_{;l}-u_{;i}\dot{x}^{\a}+uA^{k}_{i}x^{\a}_{;k}+\br{fb^{kl}u_{;l}x^{\a}_{;k}}_{;i}\\
       &\hp{=}-\br{\L_{kl}b^{lm}b^{kr}f_{;m}x^{\a}_{;r}}_{;i}+\-R^{\a}_{\b\g\d}\dot{x}^{\b}x_{;i}^{\g}\dot{x}^{\d}.}
...
\eq{\label{Ev-B-1}\dot{B}_{ij}&=u_{;ij}+T\ast\nabla u+\fr 1f \L_{ik}A^{k}_{j}+\fr 1f \-R_{\a\b\g\d}\dot{x}^{\a}x^{\b}_{;j}\dot{x}^{\g}\nu^{\d}_{;i}\\
		&\hp{=}-\fr 1f h_{ik}\br{\L_{rs}b^{sm}b^{rk}f_{;m}}_{;j}.}
}
\pf{
\eq{\dot{B}_{ij}&=-uB_{ij}+\fr 1f \dot{h}_{ik}A^{k}_{j}+\fr 1f h_{ik}\dot{A}^{k}_{j}\\
        &=-uB_{ij}-\fr 1f \br{fB_{ik}-\L_{ik}}A^{k}_{j} \\
...
\eq{u&=\fr 1f\br{\p'F\dot{s}+f^{ij}\dot{h}_{ij}-f^{mi}h^{j}_{m}\dot{g}_{ij}}\\
        &=\fr{\p'}{\p}\dot{s}+\fr 1f\br{-f^{ij}h_{ik}A^{k}_{j}+f^{ij}\L_{ij}+2f^{mj}h_{mk}A^{k}_{j}}\\
...
\eq{\label{dot u}\dot{u}&=(\log\p)''\dot{s}^2+(\log\p)'\ddot{s}+\frac{\p'}{\p}\dot{s}f^{ij}\br{B_{ij}+\fr 1f \L_{ij}}\\
  &\hp{=}+\br{g^{lj}f^{ik}+f^{ij,km}h^{l}_{m}}\br{g_{kr}A^{r}_{l}+g_{lr}A^{r}_{k}}\br{B_{ij}+\fr 1f\L_{ij}}\\
  ...
  &\hp{=}+\br{2ff^{ik}b^{jl}B_{kl}+2ff^{ij,kl}B_{kl}}\br{B_{ij}+\fr 1f\L_{ij}}\\
        &\hp{=}-ff^{ij,kl}\br{B_{kl}-\fr 1f\L_{kl}}\br{B_{ij}+\fr 1f\L_{ij}}+f^{ij}\br{\dot{B}_{ij}+\del_{t}\br{\fr 1f{\L}_{ij}}}\\
        &=(\log\p)''\dot{s}^2+(\log\p)'\ddot{s}+\frac{\p'}{\p}\dot{s}f^{ij}\br{B_{ij}+\fr 1f \L_{ij}}\\
  &\hp{=}+\br{2ff^{ik}b^{jl}B_{kl}+ff^{ij,kl}B_{kl}+f^{ij,kl}\L_{kl}}\br{B_{ij}+\fr 1f\L_{ij}}\\
  &\hp{=}+f^{ij}\br{\dot{B}_{ij}+\del_{t}\br{\fr 1f{\L}_{ij}}}\\
        &=(\log\p)''\dot{s}^2+(\log\p)'\ddot{s}+\frac{\p'}{\p}\dot{s}f^{ij}\br{B_{ij}+\fr 1f \L_{ij}}\\
  &\hp{=}+2ff^{ik}b^{jl}B_{kl}B_{ij}+ff^{ij,kl}\br{B_{kl}+\fr 1f\L_{kl}}\br{B_{ij}+\fr 1f\L_{ij}}\\
  &\hp{=}+2f^{ik}b^{jl}B_{kl}\L_{ij}+f^{ij}\br{\dot{B}_{ij}+\del_{t}\br{\fr 1f{\L}_{ij}}}.}
