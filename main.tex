...
Let us choose \(r:M\times [0,T^{\ast})\to M\) such that $\partial_tr=\operatorname{grad}_hf$ on $M$. Then defining $\check{x} (\xi, t) := x (r(\xi,t), t)$, $\check{\nu} (\xi, t) :=\nu (r(\xi,t), t)$ and $\check{f}(\xi,t):=f(r(\xi,t), t)$, we see that the flow \eqref{Flow} is equivalent to the curvature flow
\eq{\label{FlowStandard}
\check{x}&:M\times [0,T^{\ast})\to N\\
\dot{\check{x}} &= -\s \check{f} \check{\nu}.}
We call $\check{x}$ the standard parameterization as in \cite{Andrews:09/1994}.
...
\eq{
  \frac{\dot{f}}{f} &= \fr{\partial_t \check{f} - df (\partial_t r)}{f} \\
 &= \frac{\partial_t\check{f}-\check{b}(d\check{f},d\check{f})}{\check{f}}
}
