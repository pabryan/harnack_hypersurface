...
Suppose there exists a family of diffeomorphisms \(r:M\times [0,T^{\ast})\to M\), such that $r(\cdot,0)=\operatorname{id}$ and $\dot{r}=\operatorname{grad}_hf$ (\textcolor[rgb]{1.00,0.00,0.00}{in Lemma ?? we will show that under the assumptions of our main theorems such a family of time-dependent diffeomorphism exists}). Then defining $\tilde{x} (\xi, t) := x (r(\xi,t), t)$, $\tilde{\nu} (\xi, t) :=\nu (r(\xi,t), t)$ and $\tilde{f}(\xi,t):=f(r(\xi,t), t)$, we see that the flow \eqref{Flow} is equivalent to the curvature flow
...
\[
\Delta = \inf_{\gamma} \int_{t_1}^{t_2} \frac{1}{\tilde{f}} \tilde{h}(\gamma', \gamma') dt
\]
...
\]
for every $x_1, x_2$ and every $\gamma$ joining $x_1$ at $t_1$ to $x_2$ and $t_2$. The opposite inequality holds when $\tilde{f} < 0$. Then using equation \eqref{eq:exponential_harnack}, we obtain
\begin{align}
...
\end{align}
Taking supremums over all such $\gamma$, the right hand integral becomes $-\Delta$. Rearranging gives the Moser parabolic Harnack \eqref{eq:moser_harnack}. Note that the inequality reverses for $\tilde{f} < 0$, but multiplying equation \eqref{eq:exponential_harnack} by the negative number $\tilde{f}(x_1, t_1)$ reverses the inequality again.
