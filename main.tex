...
\[\del_{t}H-b^{ij}H_{;i}H_{;j}-\fr{\-R}{n+1}H+\fr{1}{2t}\geq 0,\]
...
Let us state a lemma about curvature functions which we will frequently use to estimate some of the terms in \eqref{Ev-u-new}. Recall that a curvature function $f$ is called {\it{monotone}}, if
...
\eq{\label{Speed-1}\fr{\del f}{\del g_{ij}}X_{ij}=-f^{mi}h^{j}_{m}X_{ij},\quad \fr{\del^{2}f}{\del h_{ij}\del g_{kl}}X_{ij}Y_{kl}=-(g^{kj}f^{il}+f^{ij,km}h^{l}_{m})X_{ij}Y_{kl},}
for all symmetric matrices $(X_{ij}), (Y_{ij}).$

\Theo{lemma}{CF}{
Let $F$ be a strictly monotone, $1$-homogeneous and convex curvature function. Then for all $p\neq 0$ the speed function \eqref{Speed}
satisfies
...
\eq{\label{Speed}f\cn \R_{+}\x UN &\x \Sigma^{0,2}_{+}(M)\x\Sigma^{0,2}_{+}(M)\ra \R\\	f(s,\nu,h_{ij},g_{ij})&=\p(s)\psi(\nu)\mrm{sgn}(p)F^{p}(h_{ij},g_{ij}),}
\begin{itemize}
\item $p\neq 0$,
...
% For arbitrary backgrounds, suitable support functions do not exist, but it does no harm to include $\p$, simply taking $\p \equiv 1$ whenever we do not have a suitable support function. 
...
\eq{\label{BE-gradf}\dot{V}&=\grad_{\~g}u+A(V)+uV-\fr{1}{f}(\L^{t})^{\~\sharp}V,}
\eq{\label{BE-speed} \fr{\-\n}{dt}\dot{x}=u\dot{x}-x_{\ast}\br{\grad_{\~g}u}+\fr 1f x_{\ast}\br{(\L^{t})^{\~\sharp}V }.}
...
``\eqref{BE-gradf}'': We calculate
\eq{Xu&=X\del_{t}\log|f|\\	&=\del_{t}\br{\~g(X,\grad_{\~g}\log|f|)}\\
	&=\dot{\~g}(X,V)+\~g(X,\dot{V})\\
...
\eq{\fr{\-\n}{dt}\dot{x}=&-\s\dot{f}\nu-\fr{\-\n}{dt}(x_{\ast}V)\\
				...
				=&-\s uf\nu+x_{\ast}(A(V))-x_{\ast}\br{\grad_{\~g}u}-x_{\ast}\br{A(V)}\\
				&-ux_{\ast}V+x_{\ast}\br{\fr{1}{f}(\L^{t})^{\~\sharp}V}\\
				=&u\dot{x}-x_{\ast}\br{\grad_{\~g}u}+\fr 1f x_{\ast}\br{(\L^{t})^{\~\sharp}V}
.}}
...
\Theo{lemma}{AE-A}{
\eq{\label{AE-A} \dot{A}=A^{2}+uA+\hat{\n}\grad_{\~g}u+D \grad_{\~g}u-\n \br{\fr 1f (\L^{t})^{\~\sharp}V}+\br{\overline{\mrm{Rm}}(\cdot,\dot{x})\dot{x}}^{\top}. }
%\eq{\label{AE-B} \dot{B}&=\hat{\n}^{2}_{\~g}u+ C(\cdot,\grad_{\~g}u,\cdot)+\fr 1f \L(A(\cdot),\cdot)\\
...
Therefore, using (\ref{BE-speed}) we get
\eq{x_{\ast}(\dot{A}(X))&-x_{\ast}(A^{2}(X))-\overline{\mrm{Rm}}(X,\dot{x})\dot{x}\\
&=-\-\n_{X}\br{u\dot{x}-x_{\ast}\br{\grad_{\~g}u}+\fr 1f x_{\ast}\br{(\L^{t})^{\~\sharp}V}}\\
			&=-\br{\-\n_{X}u}\dot{x}+ux_{\ast}(A(X))+x_{\ast}(\~\n_{X}\grad_{\~g}u)\\
			&\hp{=}+\~g(X,\grad_{\~g}u)\dot{x}-\-\n_{X}\br{\fr 1f x_{\ast}\br{(\L^{t})^{\~\sharp}V}}\\
			&=ux_{\ast}(A(X))+x_{\ast}(\hat{\n}_{X}\grad_{\~g}u)\\
			&\hp{=}+x_{\ast}\br{D_{X}\grad_{\~g}u}-\-\n_{X}\br{ x_{\ast}\br{\fr 1f(\L^{t})^{\~\sharp}V}}\\
			&=ux_{\ast}(A(X))+x_{\ast}(\hat{\n}_{X}\grad_{\~g}u)\\
			&\hp{=}+x_{\ast}\br{D_{X}\grad_{\~g}u}-x_{\ast}\br{\n_{X}\br{\fr 1f(\L^{t})^{\~\sharp}V}}\\
			&\hp{=}+\s h\br{X,\br{\fr 1f(\L^{t})^{\~\sharp}V}}\nu.}
		}	
%``\eqref{AE-B}'': From \eqref{BE-tildeg} and \eqref{AE-A} we calculate
%\eq{\dot{B}(X,Y)&=\fr{d}{dt}\~g(A(X),Y)\\
%			&=-B(A(X),Y)-u\~g(A(X),Y)+\fr 1f \L(A(X),Y)\\
%			&\hp{=}+\~g(A^{2}(X),Y)+u\~g(A(X),Y)+\~g(\hat{\n}_{X}\grad_{\~g}u,Y)\\
%			&\hp{=}+\~g(D_{X}\grad_{\~g}u,Y)-\~g\br{\n_{X}\br{\fr 1f \L^{\~\sharp}V},Y}\\
%			&\hp{=}+\~g(\br{\overline{\mrm{Rm}}(X,\dot{x})\dot{x}}^{\top},Y)\\
%			&=\hat{\n}^{2}_{\~g}u(X,Y)+ C(X,\grad_{\~g}u,Y)+\fr 1f \L(A(X),Y)\\
%			&\hp{=}-\~g\br{\n_{X}\br{\fr 1f \L^{\~\sharp}V},Y}+\~g(\br{\overline{\mrm{Rm}}(X,\dot{x})\dot{x}}^{\top},Y)}


\Theo{lemma}{Ev-Lambda}{
\eq{g\br{\partial_t\br{\fr{\L^{\sharp}}{f}}(X),Y}=&\fr 1f g(A(\L^{\sharp}(X)),Y)-\fr 1f \L(A(X),Y)-\fr 1f \overline{\mrm{Rm}}(\grad_{\~g}u, X,\nu, Y)\\
&+\fr{1}{f^{2}}\overline{\mrm{Rm}}\br{\br{\L^{t}}^{\~\sharp}V,X,\nu,Y}+\fr{1}{f}\-\n_{\dot{x}}\overline{\mrm{Rm}}(\dot{x},X,\nu,Y).}
}
\pf{Differentiating the defining equation
\eq{g\br{\fr{\L^{\sharp}}{f}(X),Y}=\fr 1f\L(X,Y)=\fr 1f\overline{\mrm{Rm}}(\dot{x},X,\nu,Y)}
...
\eq{g\br{\n_{Z}\br{\fr{(\L^{t})^{\~\sharp}V}{f}},Y}=&g\br{\fr{(\L^{t})^{\~\sharp}V}{f},\n_{Z}Y}-\-\n_{Z}\overline{\mrm{Rm}}(\dot{x},\cW^{-1}(Y),\nu,\dot{x})\\
...
\eq{\label{Lambdasharptilde-2}g\br{\fr{(\L^{t})^{\~\sharp}V}{f},Y}=\fr{h}{f}\br{(\L^{t})^{\~\sharp}V,\cW^{-1}(Y)}=-\L(\cW^{-1}(Y),\dot{x})}
with respect to $Z$ gives
\eq{\label{Lambdasharptilde-1}g\br{\n_{Z}\br{\fr{(\L^{t})^{\~\sharp}V}{f}},Y}
	=&-g\br{\fr{(\L^{t})^{\~\sharp}V}{f},\n_{Z}Y}-\-\n_{Z}\overline{\mrm{Rm}}(\dot{x},\cW^{-1}(Y),\nu,\dot{x})\\
...
There holds
\eq{\fr{1}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})&=\fr 1f\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr 1f\tr_{d_{h}f}\L(A(\cdot),\cdot)\\
&\hp{=}+\fr 1f\tr_{d_{h}f}\br{\tr_{h} (A\ast A)}.}
}
\pf{The claim follows from Lemma \ref{SymmetriesB}:
\eq{\tr(d_{\cW}f\circ\cW\circ A^{2})&=\tr_{d_{h}f}h(\cdot,A^{2}(\cdot))=f\tr_{d_{h}f}B(\cdot,A(\cdot))\\		&=f\tr_{d_{h}f}B(A(\cdot),\cdot)+\tr_{d_{h}f}\L(\cdot,A(\cdot))-\tr_{d_{h}f}\L(A(\cdot),\cdot)\\
					&=\tr_{d_{h}f}h(A(\cdot),A(\cdot))+\tr_{d_{h}f}\L(\cdot,A(\cdot))-\tr_{d_{h}f}\L(A(\cdot),\cdot).}
}
...
\eq{\dot{u}&-\tr_{d_{h}f}\hat{\n}_{\~g}^{2}u-\tr_{d_{h}f}C(\cdot,\grad_{\~g}u,\cdot)+\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
&+\fr 12 \tr_{d_hf} \left[\overline{\mrm{{Rm}}}(\cdot,\grad_{\~g}u)(\cdot)+ \overline{\mrm{Rm}}(\cdot, \cdot)\grad_{\~g}u\right]^{\dot{x}}\\
=&(\log\p)'' \dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	\hp{=}&+\fr 2f\tr_{d_{h}f}(\tr_{h}(A\ast A))+\fr 2f \tr_{d_{\cW}f}(A\circ\L^{\sharp}-\L^{\sharp}\circ A)\\
	\hp{=}&+\s\br{1-\fr{\tr_{d_{h}f}(h)}{f}}\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})+\fr 2f \tr_{d_{h}f}\overline{\mrm{Rm}}(\cdot,\dot{x},\dot{x},\cW(\cdot))\\
&+\fr 1f\tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\cdot,\dot{x})+\fr 1f\tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\dot{x},\cdot).}
}
\pf{Note that
\eq{u=\fr{\dot{f}}{f}=(\log \p)'\dot{s}+\fr 1f(d_{\cW}f)\dot{\cW}=(\log\p)'\dot{s}+\fr 1f (d_{\cW}f)\br{A\circ\cW+\L^{\sharp}}.}
Hence taking time derivative of both sides we arrive at
\eq{\dot{u}&=(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)\dot{\cW}\\
		&\hp{=}-\fr{u}{f}(d_{\cW}f)(A\circ\cW+\L^{\sharp})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
		&\hp{=}+\fr 1f (d_{\cW}f)\br{\dot{A}\circ\cW+A\circ\dot{\cW}+\del_{t}\L^{\sharp}}\\
		&=(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
		&\hp{=}+\fr 1f (d_{\cW}f)\br{\dot{A}\circ\cW+A^{2}\circ\cW+A\circ\L^{\sharp}-uA\circ\cW}+(d_{\cW}f)\br{\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}.}
Using that for any $T\in \mc{T}^{1,1}M,$
\eq{(d_{\cW}f)T=\tr(d_{\cW}f\circ T)}
and that $d_{\cW}f$ commutes with $\cW$ we deduce 
\eq{\dot{u}=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr 1f\tr(d_{\cW}f\circ\cW\circ(\dot{A}+A^{2}-uA))+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\
		=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})+\fr 1f\tr\br{d_{\cW}f\circ\cW\circ\br{\hat{\n}\grad_{\~g}u+D \grad_{\~g}u}}\\
	&-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\
	&+\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\br{\overline{\mrm{Rm}}(\cdot,\dot{x})\dot{x}}^{\top}}.}
Rewriting the $\grad_{\~g}$-terms we obtain 
\eq{\label{Ev-u-5-new}\dot{u}&-\tr_{d_{h}f}\hat{\n}^{2}_{\~g}u-\tr_{d_{h}f}C(\cdot,\grad_{\~g}u,\cdot)\\
		&+\fr 12 \tr_{d_hf} \left[\overline{\mrm{{Rm}}}(\cdot, \grad_{\~g}u)(\cdot)+ \overline{\mrm{Rm}}(\cdot, \cdot)\grad_{\~g}u\right]^{\dot{x}}\\
	=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	&+\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\br{\overline{\mrm{Rm}}(\cdot,\dot{x})\dot{x}}^{\top}}+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}.}		
Now we have to insert the formulas from \cref{Ev-Lambda}, \cref{Lambdasharptilde} and \cref{MixedTrace}.
Let us recollect the relevent terms seperately. Due to \cref{Ev-Lambda} we have
\eq{\label{Ev-u-1-new}&(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\ =&\fr{2}{f}\tr_{d_{\cW}f}(A\circ{\L^{\sharp}})-\fr{1}{f}\tr_{d_{\cW}f}(\L^{\sharp}\circ A)-\fr 1f\tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
\hp{=}&+\fr{1}{f^{2}}\tr_{d_{h}f}\overline{\mrm{Rm}}((\L^{t})^{\~\sharp}V,\cdot,\nu,\cdot)+\fr 1f\tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\cdot,\dot{x}).}
With \cref{Lambdasharptilde} we get
\eq{\label{Ev-u-2-new}&-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	=~&\fr{1}{f}\tr_{d_{h}f}\L(A(\cdot),\cdot)-\fr{2}{f}\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr{\s}{f}\tr_{d_{h}f}(h)\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})\\
	\hp{=}~&-\fr{1}{f}\tr_{d_{h}f}\overline{\mrm{Rm}}(\dot{x},\cdot,\dot{x},\cW(\cdot))+\fr 1f \tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\dot{x},\cdot)\\
	\hp{=}~&+\fr 1f \tr_{d_{h}f\circ\cW}\overline{\mrm{Rm}}(\dot{x},\n_{(\cdot)}(\cW^{-1}(\cdot)),\nu,\dot{x}).}
To treat the last term of $\eqref{Ev-u-2-new}$ we use the Codazzi equation:
\eq{\n_{X}(\cW^{-1}(Y))&=-\cW^{-1}(\n_{X}\cW(\cW^{-1}(Y)))+\cW^{-1}(\n_{X}Y)\\
&=-\cW^{-1}\br{(\n h(X,\cW^{-1}(Y),\cdot)^{\sharp})}+\cW^{-1}(\n_{X}Y)\\
&\hp{=}-\cW^{-1}(\overline{\mrm{Rm}}(\nu,\cW^{-1}(Y))X)^{\top}.}
Hence
\eq{\label{Ev-u-3-new}&\fr 1f \tr_{d_{h}f\circ\cW}\overline{\mrm{Rm}}(\dot{x},\n_{(\cdot)}(\cW^{-1}(\cdot)),\nu,\dot{x})\\
			=~&-\fr{1}{f}\overline{\mrm{Rm}}(\dot{x},V,\nu,\dot{x})
				-\fr 1f \tr_{d_{h}f}\L(\cW^{-1}(\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}),\dot{x})\\
			=~&\s\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})
				-\fr 1f \tr_{d_{h}f}\L(\cW^{-1}(\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}),\dot{x}).}Let us also recall \cref{MixedTrace}:
\eq{\label{Ev-u-4-new}\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})&=\fr 2f\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr 2f\tr_{d_{h}f}\L(A(\cdot),\cdot)\\						&\hp{=}+\fr 2f\tr_{d_{h}f}\br{\tr_{h} (A\ast A)}.}
Now we have to add up \eqref{Ev-u-1-new}, \eqref{Ev-u-2-new} and \eqref{Ev-u-4-new}, also taking \eqref{Ev-u-3-new} into account.
Therefore also note that from the defining equation of $(\L^{t})^{\~\sharp}V$, \eqref{Lambdasharptilde-2}, we get
\eq{\fr{1}{f^{2}}\tr_{d_{h}f}\overline{\mrm{Rm}}((\L^{t})^{\~\sharp}V,\cdot,\nu,\cdot)&=-\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}\br{\nu,\cdot,\cdot,\fr{(\L^{t})^{\~\sharp}V}{f}}\\
		&=-\fr 1f \tr_{d_{h}f}g\br{\fr{(\L^{t})^{\~\sharp}V}{f},\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}}\\
		&=\fr 1f \tr_{d_{h}f}\L(\cW^{-1}(\overline{\mrm{Rm}}(\nu,\cdot)(\cdot)^{\top}),\dot{x}).} Thus we get
\eq{\label{Ev-u-4-new}&(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	&+\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})\\
	=~& \fr 2f \tr_{d_{\cW}f}(A\circ \L^{\sharp}-\L^{\sharp}\circ A)-\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
	\hp{=}~&+\fr 1f\tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\cdot,\dot{x})+\s\br{1-\fr{\tr_{d_{h}f}(h)}{f}}\overline{\mrm{Rm}}(\dot{x},\nu,\nu,\dot{x})\\
	\hp{=}~&-\fr{1}{f}\tr_{d_{h}f}\overline{\mrm{Rm}}(\dot{x},\cdot,\dot{x},\cW(\cdot))+\fr 1f \tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\dot{x},\cdot)\\
	\hp{=}~&+\fr 2f\tr_{d_{h}f}\br{\tr_{h} (A\ast A)}.}
