...
Suppose either
\begin{enumerate}
  \item $N=\mathbb{R}^{n+1}$, $\psi$ is a positive smooth function defined on the unit sphere and the solution is strictly convex, or
  \item $N=\mathbb{R}^{n,1}$ (Minkowki space), $\psi$ is a positive smooth function defined on the hyperbolic space and the solution is co-compact, spacelike and strictly convex.
\end{enumerate}
...
\Theo{lemma}{Ev-u}{
Under the flow \eqref{Flow} we have
\eq{\label{Ev-u-1}\dot{u}-f^{ij}u_{;ij}-T\ast\n u&=(\log\p)''\dot{s}^2+(\log\p)'\ddot{s}+\frac{\p'}{\p}\dot{s}f^{ij}\br{B_{ij}+\fr 1f \L_{ij}}\\
...
\eq{\dot{w}-f^{ij}w_{;ij}-T\ast\n w&=u+t\fr{\p''}{\p}f^2-2\s t\fr{\p'}{\p}ff^{ij}B_{ij}+tf\br{2f^{il}b^{jk}+f^{ij,kl}}B_{ij}B_{kl}\\
...
Then for $p>0$ along \eqref{Flow} there holds
\eq{\label{LocHom-1}\dot{u}&\geq f^{ij}u_{;ij}+T\ast\n u+
...
\pf{We only consider $p>0.$ Note that by Lemma \ref{CF} and $u=f^{ij}\left(B_{ij}+\frac{1}{f}\Lambda_{ij}\right)$,
we have
...
\eq{\label{ev w}\dot{w}\geq& f^{ij}w_{;ij}+T\ast\n w+u+t\fr{p+1}{p}u^2-\fr{4}{p}f^{ij}g_{ij}w+\fr{4}{p+1}f^{ij}g_{ij}+\fr{2t}{p}\br{f^{ij}g_{ij}}^2\\
        =&f^{ij}w_{;ij}+T\ast\n w+\left(\fr{p+1}{p}u-\fr{4}{p}f^{ij}g_{ij}\right)w+\fr{4}{p+1}f^{ij}g_{ij}+\fr{2t}{p}\br{f^{ij}g_{ij}}^2.}
...
\[t\left(\frac{\partial_t H}{H}-n\right)+\frac{1}{2}\geq 0.\]
...
\eq{\label{xx}\dot{w}
			 -f^{ij}w_{;ij}-T\ast\n w\geq&\br{\fr{p+1}{p}u-\fr{4}{p}f^{ij}g_{ij}}w+\fr{4}{p+1}f^{ij}g_{ij}\\
            &+\fr{2t}{p}\br{f^{ij}g_{ij}}^2-2tpf^2.}
On the other hand, since $0<\kappa_i\leq 1$, we have
\[pf=\sum \frac{\partial f}{\partial \kappa_i}\kappa_i\leq f^{ij}g_{ij}\Rightarrow \fr{2t}{p}\br{f^{ij}g_{ij}}^2-2tpf^2\geq 0.\]
...
\Theo{thm}{Hyperbolic}{Let $N=\H^{n+1}$ and $F$ be a monotone, 1-homogeneous and inverse convex curvature function. Suppose $-1\leq p<0$. Then any horoconvex solution to the flow
$\dot{x}=-f\nu$ with the speed $f=-F^{p}$
satisfies
