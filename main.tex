...
\eq{
 ...
 &= \frac{\partial_t\bar{f}-\bar{b}(\bar{\nabla} \bar{f},\bar{\nabla} \bar{f})}{\bar{f}}
}
...
\eq{\label{Ev-u-5-new}\dot{u}&-\operatorname{Tr}_{d_{h}f}\~{\n}^{2}u-\operatorname{Tr}_{d_{h}f}C(\cdot,\widetilde{\grad}u,\cdot)\\
        &+\fr 12 \operatorname{Tr}_{d_hf} \left[\overline{\mrm{{Rm}}}(\cdot, \widetilde{\grad}u)(\cdot)+ \overline{\mrm{Rm}}(\cdot, \cdot)\widetilde{\grad}u\right]^{\dot{x}}\\
    =&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+\fr 1f(\log\p)'\dot{s}  df(\dot{\cW})+\fr 1f   d^2f(\dot{\cW},\dot{\cW})\\
...
In this theorem, a semicolon denotes covariant derivatives with respect to the induced metric.
...
\pf{
Again the last line of \eqref{Ev-q} is non-negative. Also note that now we have
\eq{\L=fg}
We calculate
