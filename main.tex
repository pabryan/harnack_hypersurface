...
For an open, symmetric an convex cone $\G\sub \R^n$ let $S_{\G}\sub T^{1,1}(M)$ be the set of endomorphisms with eigenvalues lying in $\G.$

For a metric $g\in T^{0,2}(M)$, $T^{1,1}_g(M)$ denotes the subbundle of $T^{1,1}(M)$ such that the fiber at $p\in M$ is the vector space of $g(p)$-self-adjoint endomorphisms.


A curvature function on $S_{\G}$ is a map
\eq{F\cn T^{1,1}(M)\cap S_{\G}\ra \R,}
such that for all metrics $g$ the restriction $F\Big|_{T^{1,1}_g(M)\cap S_{\G}}$ is smooth and
...
\eq{\~F&:T^{0,2}_{\mrm{sym},+}\x T^{0,2}(M)\to \mathbb{R}\\
...
%   \item[ii] For an open, symmetric an convex cone $\G\sub \R^n$ let $S_{\G}\sub T^{1,1}(M)$ be the set of endomorphisms with eigenvalues lying in $\G.$
  \item[i] A curvature function $F$ on $M$ is called {\it{(strictly) monotone}} on $S_{\G}$, if $d_h\~F$ is non-negative (positive) definite for all $g$ and $h$ with the property $g\ast \fr 12 (h+h^t)\in S_{\G}$.
  \item[ii] $F$ is called {\it{homogeneous of degree 1}}, if
\eq{F(\l \cW)=\l F(\cW)\quad \forall \cW\in T^{1,1}(M)\cap S_{\G},~\forall \l>0.}
  \item[iii] $F$ is called {\it{concave (convex)}} in $S_{\G}$, if for all metrics $g$ the restriction of $F$ to $S_{\G}\cap T^{1,1}_{g}(M)$ is concave (convex) at each $p\in M.$
  \item[iv] A curvature function $F$ is called {\it{inverse concave (inverse convex)}} in $S_{\G}$, if for all metrics $g$ the restriction of
\[\~F(\mc{W}):=\fr{1}{F(\mc{W}^{-1})}\]
to $S_{\G}\cap T^{1,1}_g(M)$
is concave (convex).
\end{description}
}
...
\Theo{lemma}{Lemma-CF}{
Let $\G\sub\R^n$ be a symmetric, convex and open cone. Let $F$ be monotone and $1$-homogeneous on $S_{\G}$. Then
for all $p\neq 0$, metrics $g$ on $M$ and all invertible $\cW\in S_{\G}\cap T_g^{1,1}(M)$, the curvature function $f(\cW)=\mrm{sgn}(p)F^p(\cW)$
satisfies
\eq{df(\ad(\eta)\circ \cW^{-1}\circ\eta)\geq \fr 1pf^{-1}\br{df(\eta)}^2\quad\forall \eta\in T^{1,1}(M).}

If $F$ is additionally inverse concave on $S_{\G}$, then
