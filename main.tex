...
Let us state a simple lemma about curvature functions which we will frequently use to estimate some of the terms in \eqref{Ev-u-1}.
\Theo{lemma}{CF}{
...
\eq{\dot{x}=-f\nu,} 
then, in view of $g_{ij}=\~h_{ik}\~h^{k}_{j},$ we deduce that the normal $\nu,$ viewed as a vector $\~x$ in $\R^{n+1,1}$, evolves by
