...
In the case of vanishing ambient curvature we obtain the following Harnack inequalities, which extend the well-known inequalities from \cite{Andrews:09/1994} in the Euclidean space to Minkowski space.
...
\section{Speeds depending on the support function in Euclidean space}

Up to \cref{Ev-B} none of the calculations depended on the domain of $f$. In Euclidean and Minkowski we can include dependence of $f$ on the position vector. In particular we are interested in speeds of the form
\eq{f(x,\nu,h_{ij},g_{ij})=\p(s)F(h_{ij},g_{ij}),}
where $F$ is the $p$-th power of a $1$-homogeneous and inverse concave curvature function, $0<\p\in C^{2}(\R_{+})$ and $s$ is the support function
\eq{s=\ip{x}{\nu}.}
\textcolor[rgb]{1.00,0.00,0.00}{Some of the calculations that follow are only valid when the support function is positive.}
There holds
\eq{\dot{s}=\ip{\dot{x}}{\nu}=- f}
and hence
\eq{\dot{f}&=-\p'fF+\p\dot{F}\\
        &=-\p'fF+\p F^{ij}\dot{h}_{ij}-\p F^{mi}h^{j}_{m}\dot{g}_{ij}\\
        &=-\p'fF+f f^{ij}B_{ij}.}
Thus
\eq{\label{s-1}u=-\p'F+ f^{ij}B_{ij}.}

We calculate
\eq{\dot{u}&=\fr{\p''}{\p}f^{2}-2\fr{\p'}{\p}ff^{ij}B_{ij}+ f\br{2f^{il}b^{jk}+f^{ij,kl}}B_{ij}B_{kl}+ f^{ij}u_{;ij}+T\ast \n u.}
Set 
\eq{w=tu+\fr{p}{p+1},}
then
\eq{\dot{w}&=u+tf\br{2f^{il}b^{jk}+f^{ij,kl}}B_{ij}B_{kl}+f^{ij}w_{;ij}+T\ast\n w\\
            &\hp{=}+t\fr{\p''}{\p}f^{2}-2t\fr{\p'}{\p}ff^{ij}B_{ij}\\
            &\geq u+t\fr{p+1}{p}f^{ij}B_{ij}f^{kl}B_{kl}+t\fr{\p''}{\p}f^{2}-2t\fr{\p'}{\p}ff^{ij}B_{ij}\\
            &\hp{=}+f^{ij}w_{;ij}+T\ast\n w.}
Using \eqref{s-1} we obtain
\eq{\label{eq: extension of harnack}\dot{w}&\geq \fr{p+1}{p}uw+2t\fr{p+1}{p}u\fr{\p'}{\p}f+t\fr{p+1}{p}\fr{\br{\p'}^{2}}{\p^{2}}f^{2}\\
        &\hp{=}+t\fr{\p''}{\p}f^{2}-2t\fr{\p'}{\p}fu-2t\fr{\br{\p'}^{2}}{\p^{2}}f^{2}\\
        &=\fr{p+1}{p}uw+\fr{2}{p}\fr{\p'}{\p}fw-\fr{2\p'}{(p+1)\p}f+t\fr{1-p}{p}\fr{\br{\p'}^{2}}{\p^{2}}f^{2}+t\fr{\p''}{\p}f^{2}\\
        &=\left(\fr{p+1}{p}u+\fr{2}{p}\fr{\p'}{\p}f\right)w-\fr{2\p'}{(p+1)\p}f+\fr{t}{p\p^{2}}f^{2}\br{(1-p)\br{\p'}^{2}+p\p''\p}.}
For example, if
