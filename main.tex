...
Let us state a lemma about curvature functions which we will frequently use to estimate some of the terms in \eqref{Ev-u-new}. A curvature function is a smooth map
\eq{f\cn \mc{T}^{1,1}(M)\ra \R,}
which is invariant under parallel transport.
$f$ is called {\it{monotone}}, if its derivative $df$ (which is zero along $M$ and only acts on the fibers) has only positive eigenvalues. Fixing a metric $g$, one can define
\[\~f(g,h):=f\br{g^{-1}\ast \fr 12 (h+h^t)}\equiv f(\cW). \]
Hence we have \footnote{
Let $h=h(s)$ be an arbitrary curve in $T^{0,2}_p(M)$ for a fixed $p\in M$. Then
\[\fr{d}{ds} \~f(g,h)=\fr{\del \~f}{\del h}h'(s)=\fr{\del f}{\del \cW}\cW'(s)=\fr{\del f}{\del \cW}\br{g^{-1}\ast h'(s)}=\br{\fr{\del f}{\del \cW}\ast g^{-1}}h'(s).\]}
\eq{d_h f:=\fr{\del \~f}{\del h}=\fr{\del f}{\del \cW}\ast g^{-1}=d_{\cW}f\ast g^{-1}.}
From now on, we do not distinguish $f$ and $\~f$ by different notation and write $f$ for both functions. This will not cause ambiguities, since we always indicate the derivative variable, e.g., $d_h f.$


For $T\in T^{1,1}(M)$, we have\footnote{$f^i_j T^j_i=f^{ik}g_{kj}T^j_i=f^{ik}\br{g(T(\cdot),\cdot)}_{ik}$.}
\eq{\tr(d_{\cW}f\circ T)=\tr((d_hf\ast g) \circ T)=\tr_{d_hf}g(T(\cdot),\cdot).}
...
We write $f^{ij}=\frac{\partial f}{\partial h_{ij}}$ and $f^{ij,kl}=\fr{\del^{2}f}{\del h_{ij}\del h_{kl}}$, in order, for the first and the second derivatives of $f.$
...
=&-\-\n_{X}\br{u\dot{x}-x_{\ast}\br{\grad_{\~g}u}+\fr 1f x_{\ast}\br{(\L^{t})^{\~\sharp}V}}\\
			...
			=&ux_{\ast}(A(X))+x_{\ast}(\~{\n}_{X}\grad_{\~g}u)\\
			&+x_{\ast}\br{D_{X}\grad_{\~g}u}-\-\n_{X}\br{ x_{\ast}\br{\fr 1f(\L^{t})^{\~\sharp}V}}\\
			=&ux_{\ast}(A(X))+x_{\ast}(\~{\n}_{X}\grad_{\~g}u)\\
			&+x_{\ast}\br{D_{X}\grad_{\~g}u}-x_{\ast}\br{\n_{X}\br{\fr 1f(\L^{t})^{\~\sharp}V}}\\
			&+\s h\br{X,\br{\fr 1f(\L^{t})^{\~\sharp}V}}\nu.}
		}	
...
&\hp{=}+\fr 1f\tr_{d_{h}f}h(A(\cdot),A(\cdot)).}
}
...
\eq{\label{Ev-u}\dot{u}&-\tr_{d_{h}f}\~{\n}^{2}u-\tr_{d_{h}f}C(\cdot,\grad_{\~g}u,\cdot)+\fr 1f \tr_{d_{h}f}\overline{\mrm{Rm}}(\grad_{\~g}u,\cdot,\nu,\cdot)\\
...
=&(\log\p)'' \dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	\hp{=}&+\fr 2f\tr_{d_{h}f}h(A(\cdot),A(\cdot))+\fr 2f \tr(d_{\cW}f\circ (A\circ\L^{\sharp}-\L^{\sharp}\circ A))\\
...
\eq{\dot{u}=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr 1f\tr(d_{\cW}f\circ\cW\circ(\dot{A}+A^{2}-uA))+(d_{\cW}f)\br{\fr{A\circ\L^{\sharp}}{f}+\del_{t}\br{\fr{{\L^{\sharp}}}{f} }}\\
		=&(\log\p)''\dot{s}^{2}+(\log\p)'\ddot{s}+(\log\p)'\dot{s}(d_{\cW}f)(\dot{\cW})+\fr 1f (d^{2}_{\cW}f)(\dot{\cW},\dot{\cW})\\
	&+\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})+\fr 1f\tr\br{d_{\cW}f\circ\cW\circ\br{\~{\n}\grad_{\~g}u+D \grad_{\~g}u}}\\
...
\eq{\label{Ev-u-4-new}\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})=&\fr 2f\tr_{d_{h}f}\L(\cdot,A(\cdot))-\fr 2f\tr (d_Wf\circ\L^{\sharp}\circ A)\\						&+\fr 2f\tr_{d_{h}f}h(A(\cdot),A(\cdot)),}
where we used
\begin{align}
...
\eq{\label{Ev-u-2-new}&-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	...
	\hp{=}~&+\fr 1f \tr_{d_{h}f\ast\cW}\overline{\mrm{Rm}}(\dot{x},\n_{(\cdot)}(\cW^{-1}(\cdot)),\nu,\dot{x}).}
...
Hence
\eq{\label{Ev-u-3-new}&\fr 1f \tr_{d_{h}f\ast\cW}\overline{\mrm{Rm}}(\dot{x},\n_{(\cdot)}(\cW^{-1}(\cdot)),\nu,\dot{x})\\
...
\eq{\label{Ev-u-4-new}&\fr{2}{f}\tr(d_{\cW}f\circ\cW\circ A^{2})-\fr 1f \tr\br{d_{\cW}f\circ\cW\circ\n\br{\fr 1f (\L^{t})^{\~\sharp}V}}\\
	...
	\hp{=}~&-\fr{1}{f}\tr_{d_{h}f}\overline{\mrm{Rm}}(\dot{x},\cdot,\dot{x},\cW(\cdot))+\fr 1f \tr_{d_{h}f}\-\n\overline{\mrm{Rm}}(\dot{x},\cdot,\nu,\dot{x},\cdot)+\fr 2f\tr_{d_{h}f}h(A(\cdot),A(\cdot)).}
