...
The next theorem includes our Harnack inequalities from \cite{BIS1} and presents new Harnack inequalities in de Sitter space.
...
\eq{G\cn \mc{B}_{+}(E)\x\mc{B}(E)&\ra \R\\
				(g,h)&\mt F( g^{-1}\ast \hat h). }
Then $G$ is smooth and the derivatives of $F$ and $G$ are related by 
\eq{\label{BilF-A}\fr{\del G}{\del h}(g,h)a=\tr(F'(g^{-1}\ast \hat h)\circ\hat a^{\sharp_{g}})=dF(g^{-1}\ast \hat h)\hat a^{\sharp_{g}}.}
}}
