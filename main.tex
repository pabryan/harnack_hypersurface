...
\begin{enumerate}
  ...
  \item $N=\mathbb{R}^{n,1}$ (Minkowski space), $\psi$ is a positive smooth function defined on the hyperbolic space and the solution is co-compact, spacelike and strictly convex and if $\varphi\neq 1$ then $s(\cdot,t)>0$ for all $t.$
\end{enumerate}
...
\eq{B_{ij}:=\fr{h_{ik}A^{k}_{j}}{f}=\fr{h(x_{;i},A(x_{;j}))}{f},\quad \L_{ij}:=\-R_{\a\b\g\d}\dot{x}^{\a}x^{\b}_{;i}x^{\g}_{;j}\nu^{\d},}
...
\eq{\label{Ev-Tangent-1}\del_{t}\br{b^{km}f_{;m}}=fb^{ki}u_{;i}+ub^{ki}f_{;i}+A^{k}_{j}b^{jm}f_{;m}-\L_{ij}b^{ik}b^{jm}f_{;m}.}
}
\pf{
We calculate
...
\Theo{lemma}{Ev-speed}{
Along the flow \eqref{Flow} there holds
\eq{\label{Ev-speed-1}\ddot{x}=u\dot{x}-fb^{ki}u_{;i}x_{;k}+\L_{ij}b^{jm}b^{ik}f_{;m}x_{;k}.}
}
...
\eq{\ddot{x}&=-\s \dot{f}\nu-\dot{x}_{;k}b^{ki}f_{;i}-x_{;k}\del_{t}\br{b^{ki}f_{;i}}\\
        ...
        &\hp{=}-A^{k}_{j}b^{ji}f_{;i}x_{;k}+\L_{ij}b^{jm}b^{ik}f_{;m}x_{;k}\\
...
with reversed inequality if $p<0$. Hence the result follows from the maximum principle.
}
...
satisfies the assumptions of \cref{Sphere}, which in particualr implies
\eq{\del_t\br{\Phi t^{\fr{-p}{-p+1}}}\geq 0.}
}
\subsection*{De Sitter space}

For flows of spacelike hypersurfaces in Lorentzian manifolds of nonvanishing curvature the second line of \eqref{Ev-q} can behave rather differently, since $\nu$ is timelike. Hence, in the De Sitter space of constant sectional curvature $K_{N}=1$ we obtain a simlar result as in the sphere, but only for flow with principal curvatures bounded be $1$. This is equivalent to convexity by horospheres for the dual hypersurfaces in the hyperbolic space and hence seems to be a natural assumptions for flow in the De Sitter space. 
