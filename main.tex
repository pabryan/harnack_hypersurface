...
\begin{thm}
Let $x$ be a solution of the flow, \eqref{Flow} such that $\tfrac{\partial_t f}{f} \geq -q(t)$ along the flow for some measurable function $q(t)$. Then with respect to the standard parametrisation $\check{x}$ \eqref{FlowStandard}, for all $x_1, x_2 \in M$ and $t_2 > t_1 > 0$ there holds,
\[
...
\begin{proof}
Recalling equation \eqref{eq:change_param} relating $f$ and $\check{f}$, the assumptions of the theorem imply that,
\begin{equation}
\label{eq:standard_harnack}
\frac{\partial_t\check{f} - \check{h}(\grad_{\check{h}} \check{f}, \grad_{\check{h}} \check{f})}{\check{f}} =  \frac{\partial_t\check{f}-\check{b}(d \check{f},d \check{f})}{\check{f}} \geq - q(t)
\end{equation}
...
\end{equation}
and the opposite inequality if $\check{f} < 0$.
...
&\geq Q(t_1) - Q(t_2) - \frac{1}{4} \int_{t_1}^{t_2} \frac{1}{\check{f}} \check{h}(\gamma', \gamma') dt.
\end{split}
\]
Taking exponentials,
\[
\frac{|\check{f}| (x_2, t_2)}{|\check{f}| (x_1, t_1)} \geq e^{Q(t_1) - Q(t_2)} \exp\left(- \frac{1}{4} \int_{t_1}^{t_2} \frac{1}{\check{f}} \check{h}(\gamma', \gamma')\right)
\]
for every $x_1, x_2$ and every $\gamma$ joining $x_1$ at $t_1$ to $x_2$ and $t_2$. The oppositve inequality holds when $\check{f} < 0$. Taking infimums over all such $\gamma$ and then rearranging (dropping the absolute values, and reversing the sign for $\check{f} < 0$) yields the result.
\end{proof}
