...
\Theo{rem}{}{Fix a subgroup $G$ of linear isometries that $\mathbb{H}^n/G$ is a compact hyperbolic manifold. Let $\mathcal{F}$ denote the interior of $\{\langle x,x\rangle\leq 0, x_0\geq 0\}$. A convex closed proper subset of $\mathcal{F}$ that is $G$-invariant is called $G$-Fuchsian convex body. A $G$-Fuchsian convex body has a positive support function. A Fuchsian hypersurface is the boundary of a Fuchsian convex body. Suppose now $M$ is a standard (for the group $G$), spacelike, strictly convex hypersurface that is contained in $\mathcal{F}$. Since $\nu:M\to \mathbb{H}^n$ is a diffeomorphism, $\mathbb{H}^n/G$ is a compact manifold. Hence $M$ is a $G$-Fuchsian hypersurface with positive support function.}
...
\eq{\overline{\mrm{Rm}}(X,Y,Z,W)=\-g\br{\overline{\mrm{Rm}}(X,Y)Z,W}.}
