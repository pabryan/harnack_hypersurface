...
\eq{
  \frac{\dot{f}}{f}=\frac{\partial_t\bar{f}-\bar{b}^{ij} \bar{f}_{;i} \bar{f}_{;j}}{\bar{f}}
}
...
Let us state a lemma about curvature functions which we will frequently use to estimate some of the terms in \eqref{Ev-u-1}. Recall that a curvature function $f$ is called {\it{strictly monotone}}, if
\eq{f^{ij}:=\fr{\del f}{\del h_{ij}}}
is positive definite and {\it{inverse concave}} if
\eq{\~f(\mc{W}):=\fr{1}{f(\mc{W}^{-1})}}
is concave, where we suppressed potential dependence on other arguments like $s$ and $\nu.$
For second derivatives of $f$ we write
\eq{f^{ij,kl}=\fr{\del^{2}f}{\del h_{ij}\del h_{kl} },}
...
If $F$ is only inverse concave, it was shown in \cite[p.~112]{Urbas:/1991} that
...
The motivation for our particular reparameterization is the fact that the normal does not move:
...
\end{pmatrix}
\]
where $\kappa_i$ denote the principal curvatures. In particular, $\det E = K^2.$ If $M$ is strictly convex, then $E$ is positive definite, hence invertible. In this case, the cross curvature tensor is
\[
