...
\eq{\~F&:T^{0,2}_{\mrm{sym},+}\x \O'\to C^{\infty}(M)\\
...
\eq{H_k(\mathcal{W})=\fr{1}{k!}\fr{d^k}{dt^k}\det(\id+t\mathcal{W})
\Big|_{t=0},}
for $\mathcal{W}\in T^{1,1}(M).$
Define
\[\Gamma_k(M):=\{\mathcal{W}\in T^{1,1}(M); H_1(\mathcal{W}),\cdots, H_{k}(\mathcal{W})>0\}.\]
\begin{enumerate}
  \item 
\begin{align*}H_1&\cn \Gamma_1(M)\ra C^{\infty}(M)\\
H(\mathcal{W})&=\tr(\mathcal{W}).
\end{align*}
satisfies
$dH=\id;$ therefore, $H$ is strictly monotone. The inverse concavity of $H$ follows from \cref{Lemma-CF}.
  \item 
\begin{align*}
...
\end{align*}
satisfies
$dK\Big|_{\mathcal{W}}=\det(\mathcal{W})\mathcal{W}^{-1};$ therefore, it is strictly monotone.
\item The quotients 
\begin{align*}
Q_k&:\Gamma_k(M)\to C^{\infty}(M)\\
Q_k&=\fr{H_k}{H_{k-1}}
\end{align*} 
are monotone and concave on $\G_{k}$; see \cite[Cor.~5.3]{Andrews:/2007} and \cite[Thm.~2.5]{HuiskenSinestrari:09/1999}. Furthermore they are inverse concave, cf. \cite[Cor.~2.4, Thm.~2.6]{Andrews:/2007}.
\end{enumerate}
