...
\eq{\~F(h):=F\br{g^{-1}\ast \fr 12 (h+h^t)}\equiv F(\cW). }
Hence we have \footnote{
Let $h=h(s)$ be an arbitrary curve in $T^{0,2}_p(M)$ for fixed $p\in M$. Then
\[\fr{d}{ds} \~F(g,h)=\fr{\del \~F}{\del h}h'(s)=\fr{\del F}{\del \cW}\cW'(s)=\fr{\del F}{\del \cW}\br{g^{-1}\ast\fr 12 (h'(s)+(h^t)'(s))}=\br{\fr{\del F}{\del \cW}\ast g^{-1}}h'(s),\]
since $\del F/\del \cW$ is self-adjoint when evaluated at self-adjoint operators.}
\eq{\label{dhF}d_h \~F:=\fr{\del \~F}{\del h}=\fr{\del F}{\del \cW}\ast g^{-1}=dF\ast g^{-1},}
whenever the right-hand side is evaluated at a $g$-self-adjoint operator $\cW$.
...
A curvature function $F$ defined on the invertible endomorphisms is called {\it{inverse concave}} if
...
\Theo{lemma}{CF}{
Let $F$ be a monotone, $1$-homogeneous and inverse concave curvature function. For all $p\neq 0$ and metrics $g$ on $M,$ the curvature function $f(\cW)=\mrm{sgn}(p)F^p(\cW)$
satisfies
\eq{d^2f(\eta,\eta)+2df(\eta\circ\cW^{-1}\circ\eta)\geq \fr{2}{f}\br{df(\eta)}^2\quad \forall \cW,\eta\in T^{1,1}_g(M),}
where we assumed $\cW$ is invertible.

If $F$ is convex, then for all metrics $g$ on $M$ there holds
