...
W &= \frac{1}{\lambda} W_0 \\
\nu &= \nu_0 \\
f &= \frac{1}{\lambda^p} f_0 \\
x^T &= \lambda x_0^T.
\end{cases}
\end{equation}
Here we think of $f(\xi, t) = f(W(\xi, t))$ as a smooth function $M \to \R$ and likewise for $f_0$. We also, by the usual abuse of notation, write $x$ for the position vector field in $\R^{n+1}$ at the point $x$ and denote the tangential component,
\[
Z^T = Z - \ip{Z}{\nu} \nu
\]
...
\[
h(\grad_{h} f, X) = df(X) = \frac{1}{\lambda^p} df_0 (X) = \frac{1}{\lambda^p} h_0(\grad_{h_0} f_0, X) = h\left(\frac{1}{\lambda^{p+1}} \grad_{h_0} f_0, X\right)
\]
and hence
\[
\grad_{h} f = \frac{1}{\lambda^{p+1}} \grad_{h_0} f_0
\]
Therefore,
\[
x_0^T = \frac{1}{C_0} \lambda^{p+1} \grad_{h} f .
\]
Substitution into \eqref{eq:soliton_ev_prelim} yields,
\begin{equation}
\label{eq:soliton_ev}
\begin{split}
\partial_t x &= -f(W)\nu + (\partial_t x)^T = - f(W) \nu + \frac{1}{C_0} (\partial_t \lambda) \lambda^{p+1} \grad_{h} f \\
&= - f(W) \nu - \sqrt[p+1]{1 - (p+1)C_0 t} \grad_{h} f
\end{split}
\end{equation}
recalling equation \eqref{eq:dt_lambda}. Thus we see that up to the factor $\sqrt[p+1]{1 - (p+1)C_0 t}$, the homothetic soliton parametrisation, $x(\xi, t) = \lambda(t) x_0(\xi)$ evolves by the reparametrised flow \eqref{Flow}. Defining $r_t$ to satisfy $\partial_t r_t = \sqrt[p+1]{1 - (p+1)C_0 t} \grad_{h} f$ we obtain the standard parametrisation,
\[
\tilde{x}(\xi, t) = x(r_t(\xi), t)
\]
