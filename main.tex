...
Thus for a class of inverse curvature flows we obtain some reversed Harnack inequalities. 
...
\Theo{thm}{DeSitter}{
Let $N$ be a Lorentzian spaceform with constant sectional curvature $C=1$. Let $F$ be a monotone, convex and $1$-homogeneous curvature function. Let  $p\geq 1$ and set $f=\mrm{sgn}(p)F^p$. Then along \eqref{Flow} there holds
\eq{\del_t\br{ft^{\fr{p}{p+1}}}\geq 0.}
If $p<-1$, the inequality is reversed.
}

\pf{Using \cref{CF}, in case $p>0$ \eqref{LocHom-1} becomes
\eq{\dot{u}&\geq \fr{p+1}{p}f^{ij}f^{kl}\br{B_{kl}+\fr 1f\L_{kl}}\br{B_{ij}+\fr 1f\L_{ij}}\\
		&\hp{=}-\fr 4p f^{-1}f^{ij}\L_{ij}u+\fr 2p f^{-2}f^{ij}\L_{ij}f^{kl}\L_{kl}\\
        &\hp{=}+f^{ij}u_{;ij}+T\ast\n u+\s\br{1-\fr{f^{ij}h_{ij}}{f}}\-R_{\a\b\g\d}\dot{x}^{\a}\nu^{\b}\dot{x}^{\g}\nu^{\d}\\
        &\hp{=}+\fr 2f f^{ij}h^{k}_{j}\-R_{\a\b\g\d}\dot{x}^{\a}x^{\b}_{;i}\dot{x}^{\g}x^{\d}_{;k},
        }
with the reversed inequality if $p<0.$
Again, using
...
\eq{\dot{w}&=u+t\dot{u}\\
            ...
            &\hp{=}+f^{ij}w_{;ij}+T\ast\n w}
with reversed inequality if $p<-1.$ Hence in both cases the result follows from the maximum principle. 
...
\Theo{thm}{Hyperbolic}{Let $N=\H^{n+1}$ and let $F$ be a monotone, 1-homogeneous and inverse convex curvature function. Let $p\leq -1$ or $p>1$ and set $f=\mrm{sgn}(p)F^p.$ Then along the flow
\eq{\dot{x}=-f\nu}
there holds
\eq{\del_t\br{ft^{\fr{p}{p-1}}}\leq 0,}
if $p\leq -1$ and the reverse inequality holds if $p>1.$
}
...
In case $p>1$ both inequalities reverse and hence we obtain the result.
}
